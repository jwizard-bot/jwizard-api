...
class ProcessDomainDefinitionTest {
	...
	fun `should create dev cluster path definitions`() {
		...
		val expectedPaths = listOf(devPathDefinition.format(randomPort))
	...
	fun `should create prod cluster path definitions`() {
		...
		val expectedPaths = listOf(
			prodPathDefinition.format(prefix, 0, 11),
			prodPathDefinition.format(prefix, 12, 23),
			prodPathDefinition.format(prefix, 24, 29),
		)
